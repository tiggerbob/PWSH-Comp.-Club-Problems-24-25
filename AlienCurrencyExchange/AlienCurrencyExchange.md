...
3) The total value is the sum of all the calculations of each exchange for the each number position.
4) Each calculated value in the process must be truncated to be an integer.
5) $\mid$ $x$ $\mid$ Ghoa $=$ $\mid$ $-x$ $\mid$ Ghoa, but $x$ and $-x$ have opposite signs.
<br>

## Input Format
An integer $x$ representing the amount of Ghoa.
